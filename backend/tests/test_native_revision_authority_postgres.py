...
async def test_new_install_bootstrap_replays_after_native_content_exists(tmp_path, monkeypatch):
    from app.services.native_revision_service import NativeRevisionService

    async with _fresh_database(tmp_path, monkeypatch) as (configured, dsn):
        first = await bootstrap_postgres_native(configured)
        conn = await asyncpg.connect(dsn)
        try:
            identity = NativeAuthorityIdentity.from_settings(configured)
            await consume_or_validate_native_authority(conn, identity=identity)
            vault_id = await conn.fetchval(
                "INSERT INTO vaults(name,git_path) VALUES ('bootstrap-replay','native-ledger://bootstrap-replay') "
                "RETURNING id",
            )
            service = NativeRevisionService(await postgres.get_pool())
            created = await service.create_text(
                namespace_id=vault_id, surface="document", path="retained.md", payload="Retained body",
                actor="bootstrap-proof", mutation_id=uuid.uuid4(), resource_id=uuid.uuid4(),
            )
            replay = await bootstrap_postgres_native(configured)
            assert replay["status"] == "initialized"
            assert replay["claim_id"] == first["claim_id"]
            assert replay["authority_id"] == first["authority_id"]
            assert await conn.fetchval(
                "SELECT lifecycle FROM native_resources WHERE resource_id=$1", created.resource_id,
            ) == "live"
            # A deployment image may change; the persisted initializing-image
            # receipt must not be regenerated by the provisioning tool.
            bad_receipt = configured.model_copy(update={"document_revision_runtime_image_digest": "sha256:" + "b" * 64})
            with pytest.raises(NativeAuthorityError) as error:
                await bootstrap_postgres_native(bad_receipt)
            assert error.value.code == "native_authority_claim_mismatch"
        finally:
            await conn.close()


async def test_new_install_bootstrap_resumes_claim_before_schema(tmp_path, monkeypatch):
    async with _fresh_database(tmp_path, monkeypatch) as (configured, dsn):
        conn = await asyncpg.connect(dsn)
        try:
            identity = NativeAuthorityIdentity.from_settings(configured)
            claim_id = await mint_new_database_claim(conn, identity=identity)
            assert await conn.fetchval("SELECT to_regclass('public.vaults')") is None
        finally:
            await conn.close()
        report = await bootstrap_postgres_native(configured)
        assert report["status"] == "pending"
        assert report["claim_id"] == str(claim_id)
