...
_AKB_NAME = re.compile(r"\bakb_[a-z_]+")
_PLUGIN_AKB_NAME = re.compile(r"\b(?:mcp__akb__)?(akb_[a-z_]+)")
...
def test_help_root_table_topics_are_real_help_topics() -> None:
    ...
    missing = [
        t
        for row in _table_rows(root)
        if row and (t := _backticked(row[0])) is not None
        if t not in known
    ]
    assert not missing, (
...
def test_help_root_table_tools_all_exist() -> None:
    ...
    known = _known_tools()
    ghosts: dict[str, str] = {}
    for row in _table_rows(_help_topics()[None], header_contains="| Tools |"):
        if len(row) < 2:
            continue
        topic = _backticked(row[0]) or row[0]
        for token in (t.strip() for t in row[1].split(",")):
...
def test_shipped_plugin_skills_never_name_a_nonexistent_akb_tool() -> None:
    ...
    files = [p for ext in ("*.md", "*.json") for p in _PLUGINS.rglob(ext)]
