...
every update of the checkout: `--no-build` never builds, so a missing image
fails and an old one keeps running.
...
`docker compose up -d --build`, which rebuilds it with the other built services.
A plain `up -d` builds only an image that does not exist yet and keeps an old
one. With `--no-build`, first run `docker compose build` under the same `-p`
project name. The first start on a new image recreates the PostgreSQL container
