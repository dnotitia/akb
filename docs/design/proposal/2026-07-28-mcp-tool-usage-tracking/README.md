...
6차 리뷰는 5차 수정이 만든 것을 또 잡았다 (여섯 번째).

| # | 결함 | 결과 | 수정 |
|---|---|---|---|
| 19 | `wait_for`는 취소 **승인**을 기다림 | `CancelledError`를 지연시키는 콜백이 `stop()`을 예산 밖으로 무한히 끔 (재현: 예산 30ms에 232ms) | `asyncio.wait`로 교체 — 승인을 안 기다림. 버려진 작업은 `_inflight`에 남기고 `start()`가 겹침 거부 |
| 20 | `DataError` 포착 범위가 `get_pool()`/`acquire()`까지 | 연결 획득 단계의 class-22가 **행 독약으로 오판** | 포착을 INSERT 문장으로 한정 |
| 21 | 독약 1행이 배치 전체를 죽임 | `executemany`가 원자적이라 유효 행 최대 499개 동반 폐기 | **이분 탐색으로 해당 행만 격리** (실측: 8행 중 1행만 폐기) |
| 22 | flush 실패 카운터 제거함 | 영구 환경 오류(테이블 없음·권한)가 무한 재시도인데 **포화 전까지 구분 불가** | `/health`에 복구 (폐기 판정에는 미사용) |
| 23 | `_clip()`이 값 전체를 인코딩·해시 | 100MB 인자가 **이벤트루프 58ms** | 스캔 상한 4096자 + digest 64비트로 확대 |
| 24 | `akb_unpublish(slug=,uri=)` | 핸들러는 `slug` 우선·`uri` 무시인데 URI 볼트를 기록 | slug 경로는 NULL (DB 조회 없이는 알 수 없음이 정직) |
| 25 | 비활성 시 `/health`에서 maintenance 실패 은폐 | 유지보수는 비활성에도 계속 도는데 실패가 안 보임 | 비활성 분기에도 노출 |
