...
import { Bell, BookOpen, Box, Boxes, CircleHelp, Compass, File, FilePlus2, FileText, GitCommitHorizontal, GitGraph, House, Plus, Search, Settings, Share2, Table2, Users, type LucideIcon } from "lucide-react";
...
export function AppPageLocation({ isAdmin = false, mobile = false }: { isAdmin?: boolean; mobile?: boolean }) {
  ...
  return (
    <nav aria-label="Current page" data-slot="app-page-location" className={className}>
      {vault && <><Link to={`/vault/${encodeURIComponent(vault)}`} title={vault} className="inline-flex min-w-0 max-w-[50%] items-center gap-2 rounded-[var(--radius-sm)] text-foreground-muted hover:text-link focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring">
        <Box className="h-4 w-4 shrink-0" aria-hidden /><span className="truncate">{vault}</span>
      </Link><span aria-hidden className="text-subtle">/</span></>}
