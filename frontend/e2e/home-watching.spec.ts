...
for (const width of [1920, 1440, 375]) for (const dark of [false, true]) {
  test(`Home watching ${width}px ${dark ? "dark" : "light"}`, async ({ page }, testInfo) => {
    ...
    const recent = page.getByRole("region", { name: "Watched document updates", exact: true });
    await expect(page.locator("#recent").getByText("Another recent document")).toBeVisible();
    const link = recent.getByRole("link", { name: "Preview Watched operating guide", exact: true });
    await expect(link).toBeVisible();
    await link.hover();
    await expect(link).toHaveCSS("background-color", "rgba(0, 0, 0, 0)");
    await expect(link.locator("..")).toHaveCSS("background-color", dark ? "rgb(31, 41, 53)" : "rgb(240, 242, 245)");
    ...
    await recent.getByRole("link", { name: "Open Watched operating guide in fixture vault", exact: true }).click();
    await expect(page.getByRole("region", { name: "Document workspace" })).toHaveAttribute("data-presentation", "page");
    ...
    await expect(recent.getByRole("heading", { name: "Watched documents", exact: true })).toBeFocused();
    expect(writes).toEqual(["DELETE /api/v1/notification-subscriptions"]);
    await page.reload();
    await expect(recent.getByText("No watched updates yet")).toBeVisible();
  });
}
